...
var kb kh y cb ch Q n phi spread R y_gross d 
...
parameters beta alpha theta gamma Z_ss k w sigma wh s f rho_z sigma_eps mc
...
d_ss=(kb_ss-N_ss)/beta;

Q_run_ss=(beta/(1-beta))*(Z_ss-alpha*Kbar)-alpha*Kbar;
Q_bar_ss=f*(d_ss/kb_ss)-Z_ss;     %f*Q_ss*(1-1/phi_ss)*R_ss-Z_ss;
...
(y)=(y_gross)-(alpha/2)*((kh))^(2)-mc*Q(-1)*kb(-1);                 
...
(n)=   sigma*(  ( Z_ss*exp(z)+(Q)-mc*Q(-1) )*(kb(-1)) - (d(-1)) )  + w;
